...
class GameInprogressScreenState extends State<GameInprogressScreen>
  ...
  @visibleForTesting
  bool get saveErrorForTesting => _saveError;
  ...
  Future<void> _updateGame() async {
    ...
    if (loggedInUser != null) {
      try {
        ...
        if (_saveError && mounted) {
          setState(() => _saveError = false);
        }
      } catch (e) {
        Utilities.debugPrintWithCallerInfo("Error saving game to database: $e");
        if (mounted && !_saveError) {
  ...
  Widget _buildPlayerCards() {
    return ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        int playerScore =
            pgi.scores.length >= currentHole ? pgi.scores[currentHole - 1] : 0;
