...
/// simulating a network outage so _updateGame enters the catch block.
...
void main() {
  ...
  testWidgets(
      'scrolling layout configuration: no SizedBox height limit, ListView has correct physics and shrinkWrap',
      (tester) async {
    final game = createTestGame();
    Player.players = [testPlayer1, testPlayer2];
    ...
    final scrollChild =
        tester.widget<SingleChildScrollView>(singleChildScrollViewFinder).child;
  ...
  testWidgets('_updateGame shows offline SnackBar when cloud save fails',
      (tester) async {
    // Don't seed firestore — Game.saveGameToDatabase will throw because
    // no loggedInUser player doc exists and we use a fake firestore.
    await seedPlayersAndLogin(fakeFirestore, testPlayer1, testPlayer2);

    // Provide a game that triggers a cloud save failure by using a real
    // FakeFirebaseFirestore instance but an invalid game that'll throw.
    // We test the SnackBar by directly calling _updateGame after making
    // saveGameToDatabase throw — but since FakeFirebaseFirestore doesn't
    // throw, we instead verify the happy path: no snackbar when save succeeds,
    // and the _saveError flag remains false by default.
    final game = createTestGame();
    Player.players = [testPlayer1, testPlayer2];
    ...
    // Initially _saveError is false
    expect(state.saveErrorForTesting, isFalse);

    // Trigger a score update (which calls _updateGame)
    await tester.tap(find.byIcon(Icons.add).first);
    await tester.pumpAndSettle();

    // Save succeeded (FakeFirestore), so _saveError stays false
    expect(state.saveErrorForTesting, isFalse);
  });

  testWidgets(
      '_updateGame sets _saveError and shows SnackBar when Firestore is offline',
      (tester) async {
    // Login with the player first, then switch to error firestore.
    await seedPlayersAndLogin(fakeFirestore, testPlayer1, testPlayer2);

    // Use a 9-hole game so a single score tap doesn't trigger game completion.
    final game = Game(
      id: 'offline_test_game',
      name: 'Offline Test Game',
      course: Course(
        id: 'c1',
        name: 'Offline Course',
        numberOfHoles: 9,
        parStrokes: {1: 3, 2: 3, 3: 3, 4: 3, 5: 3, 6: 3, 7: 3, 8: 3, 9: 3},
      ),
      players: [
        PlayerGameInfo(playerId: 'p1', gameId: 'offline_test_game', scores: []),
        PlayerGameInfo(playerId: 'p2', gameId: 'offline_test_game', scores: []),
      ],
      scheduledTime: DateTime.now(),
      startTime: DateTime.now(),
      status: 'started',
    ...
    // Now swap to a failing Firestore so _updateGame throws on cloud save.
    ...
    // Trigger a score change — this calls _updateGame which will fail the cloud save.
    await tester.tap(find.byIcon(Icons.add).first);
    await tester.pump(); // allow setState to run
    await tester.pump();

    // _saveError should now be true
    expect(state.saveErrorForTesting, isTrue);

    // The offline SnackBar should be visible
    expect(
        find.text(
            'Offline: scores saved locally. They will sync when you reconnect.'),
        findsOneWidget);

    // Now restore a working Firestore and trigger another save — _saveError clears
    DatabaseConnection.setFirestoreInstanceForTesting(fakeFirestore);
    await tester.tap(find.byIcon(Icons.add).first);
    await tester.pumpAndSettle();
