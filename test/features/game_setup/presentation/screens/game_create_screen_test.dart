...
void main() {
  ...
  testWidgets(
      'shows warning dialog when active game exists and cancel stops creation',
      (tester) async {
    final state = await pushAndGetState(tester);
    state.setSelectedCourseForTesting(_fakeCourse());
    state.setSelectedPlayersForTesting([
  ...
  testWidgets(
      'shows warning dialog when active game exists and continue creates game',
      (tester) async {
    final state = await pushAndGetState(tester);
    state.setSelectedCourseForTesting(_fakeCourse());
    state.setSelectedPlayersForTesting([
  ...
  testWidgets(
      'authenticated user: schedules game in the future, sets unstarted_game status and pops to dashboard',
      (tester) async {
    UserProvider().loggedInUser = _authPlayer();
    final state = await pushAndGetState(tester);
    state.setSelectedCourseForTesting(_fakeCourse());
