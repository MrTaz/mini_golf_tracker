...
import 'package:mini_golf_tracker/features/navigation/presentation/widgets/past_games_list_view.dart';
...
void main() {
  ...
  testWidgets('PastGamesListView throws when not logged in', (tester) async {
    await tester
        .pumpWidget(MaterialApp(home: Scaffold(body: PastGamesListView())));
  ...
  testWidgets(
    ...
    final game1 = Game(
      ...
      name: 'Test Game 1',
      course: Course(
          id: 'c1', name: 'Pinecrest', numberOfHoles: 1, parStrokes: {1: 3}),
      players: [
    ...
    final game2 = Game(
      ...
      name: 'Test Game 2',
      course: Course(
          id: 'c2', name: 'Oakridge', numberOfHoles: 1, parStrokes: {1: 3}),
      players: [
  ...
  testWidgets('PastGamesListView catches load errors and sets _isLoading false',
      (tester) async {
    _setLargeScreen(tester);
  ...
  testWidgets(
    ...
    final game = Game(
      ...
      name: 'Error Game',
      course: Course(
          id: 'c1', name: 'Error Hills', numberOfHoles: 1, parStrokes: {1: 3}),
      players: [
        PlayerGameInfo(playerId: 'creator1', gameId: 'g1', scores: [2]),
      ],
      scheduledTime: DateTime(2026, 1, 1),
  ...
  testWidgets(
      'PastGamesListView falls back to local cache when Firestore fails',
      (tester) async {
    _setLargeScreen(tester);
    ...
    // Seed SharedPreferences with a completed local game for creator1
    final localGame = Game(
      id: 'local_g1',
      name: 'Local Cached Game',
      course: Course(
          id: 'c_local',
          name: 'Local Hills',
          numberOfHoles: 1,
          parStrokes: {1: 3}),
      players: [
        PlayerGameInfo(playerId: 'creator1', gameId: 'local_g1', scores: [2]),
      ],
      scheduledTime: DateTime(2026, 1, 1),
      startTime: DateTime(2026, 1, 1),
      completedTime: DateTime(2026, 1, 1),
    ...
    // Make Firestore throw so we hit the offline fallback path
    DatabaseConnection.setFirestoreInstanceForTesting(MockFirestoreWithError());
    ...
    // The locally cached game should be shown
    expect(find.text('Local Hills'), findsOneWidget);
  });

  testWidgets(
      'PastGamesListView handles double failure (Firestore + local cache)',
      (tester) async {
    _setLargeScreen(tester);
    ...
    DatabaseConnection.setFirestoreInstanceForTesting(MockFirestoreWithError());
    // Clear SharedPreferences so local load returns an empty list (no localError)
    SharedPreferences.setMockInitialValues({});
    ...
    // Empty state message shown with offline text
    expect(find.textContaining("We couldn't sync your past games"),
        findsOneWidget);
  });
}
