...
import 'package:mini_golf_tracker/features/auth/data/models/contact_identity.dart';
...
void main() {
  ...
  group('Player.createPlayer behavioral tests', () {
    ...
    test('verifies nickname-only creation (empty playerName)', () async {
      final player =
          await Player.createPlayer('', 'NickOnly', ownerId: 'owner1');
      expect(player.playerName, '');
  ...
  group('Security Guardrail Tests (Phase 5.1 & Phase 5.2)', () {
    ...
    test('resolveCanonicalPlayer throws Contact Conflict for split-identity',
        () async {
      ...
      expect(
        () => Player.resolveCanonicalPlayer(splitPlayer,
            ownerIdForNewPlayer: 'owner-new'),
        throwsA(isA<DatabaseConnectionError>()
            .having((e) => e.message, 'message', 'Contact Conflict')),
      );
    });

    test(
        'claimPlayerForVerifiedAuthUser and claimPlayerForAuthUser throw Contact Conflict for split-identity',
        () async {
      await Player.createPlayer(
        'Player A',
        'PA',
    ...
    test(
        'canVerifiedAuthUserClaimPlayer returns false for already claimed player by different UID',
        () async {
    ...
    test(
        'canVerifiedAuthUserClaimPlayer returns true if claimed_by_uid matches current uid',
        () async {
      final claimedPlayer = Player(
        id: 'p-claimed',
        playerName: 'Claimed',
    ...
    test(
        'claimPlayerForVerifiedAuthUser rejects claim for already claimed player by different UID',
        () async {
      await Player.createPlayer(
        'Claimed Player',
        'CP',
    ...
    test(
        'claimPlayerForAuthUser throws error for already claimed player by different UID',
        () async {
      await Player.createPlayer(
        'Claimed Player',
        'CP',
      ...
      expect(
        ...
        throwsA(isA<DatabaseConnectionError>()
            .having((e) => e.message, 'message', 'Player already claimed')),
      );
    });

    test(
        'claimPlayerForVerifiedAuthUser returns null when canVerifiedAuthUserClaimPlayer returns false due to contact mismatch',
        () async {
      // Create a reservation that points to a player with a different email
      final reservationId =
          ContactIdentity.reservationIdForEmail('a@example.com');
  ...
  group('Player isQuickPlay unit tests', () {
    ...
    test('createPlayer sets isQuickPlay and resolves canonical player',
        () async {
      final db = FakeFirebaseFirestore();
      DatabaseConnection.setFirestoreInstanceForTesting(db);

      final created =
          await Player.createPlayer('QP Player', 'QP', isQuickPlay: true);
      expect(created.isQuickPlay, isTrue);

      final resolved = await Player.resolveCanonicalPlayer(created,
          ownerIdForNewPlayer: 'owner-x');
  ...
  group('Player createPlayer ownerId defaults coverage tests', () {
    test('createPlayer with active Firebase apps and matching currentUser UID',
        () async {
    ...
    test(
        'createPlayer when auth currentUser getter throws exception defaults to guest',
        () async {
      TestWidgetsFlutterBinding.ensureInitialized();
      final originalFirebase = FirebasePlatform.instance;
      final originalAuth = FirebaseAuthPlatform.instance;
  ...
  group('Friends Caching Tests', () {
    ...
    test(
        'loadUserPlayers fetches from Firestore and caches to SharedPreferences',
        () async {
      final friend = Player(
        id: 'friend-1',
        playerName: 'Friend One',
    ...
    test(
        'loadUserPlayers falls back to SharedPreferences cache when Firestore throws error',
        () async {
    ...
    test(
        'addPlayerFriend saves the updated friends list to SharedPreferences cache',
        () async {
