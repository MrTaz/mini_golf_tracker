...
void main() {
  ...
  testWidgets('Address normalized substring match in conflict check',
      (tester) async {
    await fakeFirestore.collection('courses').add({
  ...
  testWidgets('Duplicate not triggered if coordinates are > 100 meters away',
      (tester) async {
    tester.view.physicalSize = const Size(800, 2500);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
  ...
  testWidgets('displays duplicate dialog on save when exact duplicate exists',
      (tester) async {
    tester.view.physicalSize = const Size(800, 2500);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
    ...
    expect(
        find.descendant(
            ...
            matching: find.text('Duplicate Course')),
        findsOneWidget);
    expect(
        find.text(
  ...
  testWidgets('filters out self coordinates during conflict check when editing',
      (tester) async {
    tester.view.physicalSize = const Size(800, 2500);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
    ...
    expect(
        savedCourses.docs
            .any((d) => d.get('name') == 'Chucksters Fire Tower Edited'),
        isTrue);
  });

  group('Behavioral tests', () {
    testWidgets(
        'findConflictingCourses Haversine threshold triggers within 100 meters',
        (tester) async {
      tester.view.physicalSize = const Size(800, 2500);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
    ...
    testWidgets('normalized address substring matching triggers conflict',
        (tester) async {
      tester.view.physicalSize = const Size(800, 2500);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
