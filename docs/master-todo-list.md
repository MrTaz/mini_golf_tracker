...
* [x] **Fix Offline Firestore Connection Timeout Crash:** Configured `loadUserPlayers()` with a 2-second timeout to fall back to the SharedPreferences cache when Firestore is unreachable. Safe-guarded all invocations of `loadUserPlayers()` inside `UserProvider` (initialization, login, and auth state listener) using try-catch blocks to prevent startup crashes when offline.
