...
* [x] **Social Auth E2E Tests:** Write Patrol integration tests covering the native OS-level pop-ups for Apple and Meta/Facebook authentications.
...
* [ ] **Google Play CI/CD Integration:** Update the GitHub Actions workflow to automatically upload the Android `.aab` release build to the Google Play Console (e.g., to the Internal Testing track).
...
* [x] **Side Menu & Dashboard Routing:** Fix the navigation trap for logged-in users accessing "Friends" or "Past Games" from the `AppDrawer`. (Either update the `AppDrawer` to change the `DashboardScreen` tab index, or use `Navigator.canPop(context)` to conditionally render the `AppBar` with a back button). *Note: Social login race conditions were globally centralized.*
...
* [x] **Fix Authenticated Friends Persistence & Blank Scorecards:** Update the friends list fetching logic in `UserProvider` to explicitly call `loadUserPlayers()` upon login and app initialization. Cache authenticated friends to `SharedPreferences` (similar to `guest_players`) or ensure Firestore offline persistence hydrates the `Player.players` list. This will fix the bug where in-progress scorecards show blank names because `getPlayerFriendById()` is returning empty profiles.
* [x] **Fix Active Game Scrolling:** In `lib/game_inprogress_screen.dart`, locate the `SingleChildScrollView` in the `build()` method. Remove the hardcoded `SizedBox(height: MediaQuery.of(context).size.height + 200)` that artificially restricts its height. Additionally, ensure the nested `ListView.builder` inside `_buildPlayerCards()` is set to `shrinkWrap: true` and `physics: const NeverScrollableScrollPhysics()` so the entire screen scrolls naturally to reveal all players.
* [x] **Fix False Dashboard Rankings:** In `lib/players_card_widget.dart`, locate the `getPlayerCards()` method where the `PlayerProfileWidget` is instantiated. Currently, it passes the loop index `i` directly as the `rank`. Update this logic so that the rank is only passed if the player actually has a score: `rank: player.totalScore > 0 ? i : null`.
* [x] **Fix Dashboard Game Card Taps:** In `lib/game_card_widget.dart`, the `ListTile` representing a saved game currently lacks an `onTap` handler. Add an `onTap` callback that pushes `GameInprogressScreen(currentGame: game)` if the game status is "started", and pushes `GameStartScreen(unstartedGame: game)` if the status is "unstarted_game".
* [x] **Fix Offline Firestore Connection Timeout Crash:** Configured `loadUserPlayers()` and `fetchGamesForCurrentUser()` with 2-second timeouts to prevent startup and dashboard navigation hangs when Firestore is unreachable. Parallelized participant game document fetches using `Future.wait` and safe-guarded all provider invocations of `loadUserPlayers()` with try-catch blocks to ensure robust offline operation.
